...
runid='R113_LTE1R97_CF5_flow1p2'; % Previous run id
runid='R116_LTEinc113_CF5_flow1p2'; % Previous run id
pathrun=['C:/Users/Liujuan.Tang/Documents/VDatum/WestCoast/runoutput/' runid '/adcirc_output/'];

fn_grd=[pathrun 'fort.14'];
infile53=[pathrun 'fort.53'];
pathin='../s1_prep/out/';
pathout=['out/' runid '/'];
%----------------
...
DAS_list={'DA_NO'; 'DA_BC';'DA_TP';'DA_BC_TP';...
    'DA_CE';'DA_BC_CE'};
DAS=DAS_list{3}; % DAS=DAS_list{4}; % DAS=DAS_list{4}; % only 2 option is used here, DAS=DAS_list{4} or DAS=DAS_list{2};
...
% load /disks/NASUSER/lshi/file_for_Ed/LTEs/unified/test_bc_in/code_2020_04_25/wcadcirc/processing/exstn_seq_155.dat;
% whichstn=[1:155];% include all 149 station from Rachel plus 6 deep water sensor
% whichstn=exstn_seq_155;

%------------------
% load coops data with 
eval(['load ' pathin 'STNAMPPHA.mat']) %TPC_list_adcirc
%TPC_list_adcirc={'K1';'O1';'P1';'Q1';'M2';'S2';'N2';'K2'};% the list for ADCIRC
TPC_list_adcirc;
cent01=load('../../../indata/alpha_coops_37HC.mat');
alpha=cent01.alpha;clear cent01; % aHC (size 4x37), include 1) name, 2) angular speed (unit: degree/hour), 3) angular speed (unit: /s), 4) nodal factor, amplitude (non-dimensional), 5) nodal factor, phase (degree)
TPC_list_alpha=alpha(1,:);
aHC={};
for i=1:length(TPC_list_adcirc)
    temp=TPC_list_adcirc{i};
    iiTPC=temp;%[temp(1) '(' temp(2) ')' ];
    loc=strmatch(iiTPC,TPC_list_alpha);
    aHC(:,end+1)=alpha(:,loc);
end
%aHC(4,:)'
whichHC=[1:8];
...
%HC5153=read_in_fort_51_53(infile53,pathout); % HC5153 (size(mesh node size x 2x(no of HC)), in wc case no of HC is has 13: K1,O1,P1,Q1,M2,S2,N2,K2,M4,M6,M8,S4,S6
eval(['load ' pathout '/HC5153'])
%--------------------
which53HC=[];
for i=1:length(TPC_list)
    temp=TPC_list{i};
    loc=strmatch(temp,{hc53.name});
    which53HC(end+1)=loc;
end

%------------------------------
HCAMPstn=HC5153(whichnode,(which53HC-1)*2+1);
HCPHAstn=HC5153(whichnode,(which53HC-1)*2+2);
HCAMP=HC5153(:,(which53HC-1)*2+1);
HCPHA=HC5153(:,(which53HC-1)*2+2); % this model HC field to be save somewhere
...
%whichofwhichHC=[3:3];
whichofwhichHC=iTPC;%[3:3];

whichHCin=cell(length(whichofwhichHC),6);
%for i=[3:3] %Select TIDAL POTENTIAL CONSTITUENT
for ii=1:length(whichofwhichHC) %Select TIDAL POTENTIAL CONSTITUENT
    i=whichofwhichHC(ii) %Select TIDAL POTENTIAL CONSTITUENT
    TPC=TPC_list{i},   % NAME OF TIDAL POTENTIAL CONSTITUENT
    fprintf('%s : %d of %d\n',TPC, ii ,length(whichofwhichHC) );
    ...
    whichHCin{ii,1}=HCin;
    whichHCin{ii,2}=HCAMP(:,i);% ith HC, amplitude
    whichHCin{ii,3}=HCPHA(:,i);% ith HC, phase

    %---------------------------------------------------------
    tStart=tic;
    [dxb, dxf, dxc]=LTEs(HCin,fn_grd, fn_err,NLI, NLI_MFO, ERR, DAS, TPC,ICS,Ae_method,CTIP,testcase,internal_inverse,dis_method,CITE,pathout); % dxb, xdf is incremental for BC and tidal potential 
%     dxincr{i,1}=dxb;dxincr{i,2}=dxf;dxincr{i,3}=dxc;
% %    LTEs(fn_grd, fn_err,TPC,ICS,Ae_method,CTIP,testcase,internal_inverse,dis_method,CITE);
%         whichHCin{ii,4}=dxb;
%         whichHCin{ii,5}=dxf;
%         whichHCin{ii,6}=dxc; 
   eval(['save ' pathout '/dxincr_dxb_dxf_r01_'  TPC ' dxb dxf dxc'])
   disp(['************** total time ',num2str(toc(tStart)),' sec*******']);

end
eval(['save ' pathout '/whichHCin whichHCin DAS'])

% save dxincr_dxb_dxf_r02_nscale_1_0_R0_003_exstn.mat dxincr;% save dxincr_dxb_dxf_r12_nscale_1_0_R0_0003.mat dxincr;% just a name, sometime add the value of parameters to differentiate it
% save dxincr_dxb_dxf_r05_nscale_1_0_R0_003_exstn.mat dxincr;% save dxincr_dxb_dxf_r12_nscale_1_0_R0_0003.mat dxincr;% just a name, sometime add the value of parameters to differentiate it
% save dxincr_dxb_dxf_r01_nscale_1_0_R0_003_exstn_test_coeffs_normal_scale_04.mat dxincr;% save dxincr_dxb_dxf_r12_nscale_1_0_R0_0003.mat dxincr;% just a name, sometime add the value of parameters to differentiate it
%eval(['save out/dxincr_dxb_dxf_r01_'  TPC ' dxb dxf dxc'])


%save /disks/NASUSER/lshi/file_for_Ed/LTEs/unified/test_bc_in/code_2020_04_25/wcadcirc/a_2021_03_04/test05_02/dxincr_dxb_dxf_r01_nscale_1_0_R0_003_exstn_test_coeffs_large_scale.mat dxincr;% save dxincr_dxb_dxf_r12_nscale_1_0_R0_0003.mat dxincr;% just a name, sometime add the value of parameters to differentiate it
%save /disks/NASUSER/lshi/file_for_Ed/LTEs/unified/test_bc_in/code_2020_04_25/wcadcirc/a_2021_03_04/test05_02/whichHCin.mat whichHCin;
%----------------------------------------------------------------------  
...
xabc=[];
    ...
    if internal_inverse
        ...
        %D2=X;
        g=9.81;
        % MCO=max(H,1);% MCO=max(H,1).*sqrt(g*max(H,1))this is no good;% 
        MCO=sqrt(g*max(H,1));
        MCOa=mean(MCO(meshele(:,2:4)),2);    
%         reference_diff_co=mean(MCOa);
%         MCOa=reference_diff_co*ones(size(MCOa));
    end
...
function     [dxb, dxf, dxc]=LTEs(HCin,fn_grd, fn_err,NLI, NLI_MFO, ERR, DAS, TPC,ICS,Ae_method,CTIP,testcase,internal_inverse,dis_method,CITE,pathout);
...
dxb=[];dxf=[];dxc=[]; % do we need to add dxc into the code?
...
switch CF
    case 5
        C=0.003./max(H,1);  % this is too big for O1 in Bohai Sea
         ...
         C=0.00100./max(H,1); % I use this for O1 in Bohai Sea, LTE 
...
Aele=elementspernode01a(meshele); % Aele is a lnodes x leles size sparse matrix
corrPSI=zeros(lnodes,1);
...
Aenodr=3./(Aele*Aee);   % Aenod is the control area of a node, 1/3 of sum of surrending element area
                        % Aenodr is the reverse of Aenod
                        % it will be feed into random subroutine for now
%                        Aenodr=mean(Aenodr)*ones(size(Aenodr));
sAenodr=sparse([1:lnodes],[1:lnodes],Aenodr,lnodes,lnodes);clear Aenodr;
%
...
%        relaxation_scale=nscale*100000; % relaxation_scale=100000; % meter
%        relaxation_scale=nscale*200000; % relaxation_scale=100000; % meter
%        relaxation_scale=nscale*120000; % relaxation_scale=100000; % meter
%        relaxation_scale=nscale*60000; % relaxation_scale=100000; % meter
%        relaxation_scale=nscale*2; % relaxation_scale=100000; % meter
%        relaxation_scale=nscale*40000; % relaxation_scale=100000; % meter
%         relaxation_scale=nscale*20000; % relaxation_scale=100000; % meter
%        relaxation_scale=nscale*6000; % relaxation_scale=100000; % meter
%        relaxation_scale=nscale*2000; % relaxation_scale=100000; % meter
        relaxation_scale=nscale*2; % relaxation_scale=100000; % meter
...
%         half_dt_iter=24;%100;
%         half_dt_iter=80;%100;
%         half_dt_iter=400;%100;
%         half_dt_iter=100;%100;
         %half_dt_iter=1;%100;
...
%        [norm01]=std_random_method(dt,half_dt_iter,A6,D12,lnodes,whichnode,nsta,meshele,meshnod);
        [norm01]=std_random_method_CETP03(dt,half_dt_iter,A6,D12,lnodes,whichnode,nsta,sAenodr,meshele,meshnod,pathout);
eval(['save ' pathout 'norm01.mat norm01']);
        
jackknifing=1;

if jackknifing; % it only works for strcmp(DAS,'DA_in') and strcmp(DAS,'DA_BC_TP') 
...
% jFeq=Feq;
% jbcs=bcs;
Jnsn=find_surrounding_node(meshele,Jwhichnode);
...
%%%        nbcs=jbcs;
        for kkk=1:1; % k has to be 1 for now, sicen it need baseline PSI % loop for k really not necessary since for nom-linear model, we only interation once. 
    ...
    dxb=[];dxf=[];dxc=[];
    [dxb,kent04,rep,cent05,kent06]=DA_diff_incr_BC_more_output(dd,dt,half_dt_iter,sAenodr,A6,D12,norm01,lnodes,opnod,kki,HH,HT,A11,nA12,uncerB,R);
%    [dxb]=DA_diff_incr_BC(dd,dt,half_dt_iter,sAenodr,A6,D12,norm01,lnodes,opnod,kki,HH,HT,A11,nA12,uncerB,R);

    dxbPSI=[];dxfPSI=[];dxcPSI=[];
	[dxbPSI]=LTE_fwd_BC_TP_CE(dxb,[],[],opnod,kki,A11,nA12,[],[]);
	[dxfPSI]=LTE_fwd_BC_TP_CE([],dxf,[],opnod,kki,A11,[],Af1,[]);
	[dxcPSI]=LTE_fwd_BC_TP_CE([],[],dxc,opnod,kki,A11,[],[],nA61);
    %T save('/disks/NASUSER/lshi/file_for_Ed/LTEs/unified/test_bc_in/code_2020_04_25/wcadcirc/a_2021_03_04/test05_02/DA_BC.mat','dxb','dxf','dxc','dxbPSI','dxfPSI','dxcPSI','dd','HH','kent04','rep','cent05','kent06','-v7.3');
    %save('out/DA_BC.mat','dxb','dxf','dxc','dxbPSI','dxfPSI','dxcPSI','dd','HH','kent04','rep','cent05','kent06','-v7.3');
    eval(['save ' pathout '/DA_BC.mat dxb dxf dxc dxbPSI dxfPSI dxcPSI dd HH kent04 rep cent05 kent06 -v7.3'])
    %    
%save('/disks/NASUSER/lshi/file_for_Ed/LTEs/unified/test_bc_in/code_2020_04_25/wcadcirc/a_2021_03_04/test05_02/DA_BC.mat','dxb','dxf','dxc','dxbPSI','dxfPSI','dxcPSI')
        end;

    elseif DASALL==2
% % % elseif strcmp(DAS,'DA_TP');
    % the most important decision is which approach, 
    % in this particular application, we decide to use
    % solution ?. 
    % change the tidal potential only
...
%        R0=0.0003;
        ...
        % ok here we start the iteration
            ...
            dxb=[];dxf=[];dxc=[];
%                    [dxf,kent04,rep,cent05,kent06]=DA_diff_incr_TP_more_output(dd,dt,half_dt_iter,sAenodr,A6,D12,norm01,lnodes,opnod,kki,HH,HT,A11,Af1,uncer,R);
                    [dxf,kent04,rep,cent05,kent06]=DA_diff_incr_TP_more_output_singular(dd,dt,half_dt_iter,sAenodr,A6,D12,norm01,lnodes,Jwhichnode,Jnsn,opnod,kki,HH,HT,A11,Af1,uncer,R);
                       
    dxbPSI=[];dxfPSI=[];dxcPSI=[];
	[dxbPSI]=LTE_fwd_BC_TP_CE(dxb,[],[],opnod,kki,A11,nA12,[],[]);
	[dxfPSI]=LTE_fwd_BC_TP_CE([],dxf,[],opnod,kki,A11,[],Af1,[]);
	[dxcPSI]=LTE_fwd_BC_TP_CE([],[],dxc,opnod,kki,A11,[],[],nA61);
    save('/disks/NASUSER/lshi/file_for_Ed/LTEs/unified/test_bc_in/code_2020_04_25/wcadcirc/a_2021_03_04/test05_02/DA_TP.mat','dxb','dxf','dxc','dxbPSI','dxfPSI','dxcPSI','dd','HH','kent04','rep','cent05','kent06','-v7.3');
%    save('/disks/NASUSER/lshi/file_for_Ed/LTEs/unified/test_bc_in/code_2020_04_25/wcadcirc/a_2021_03_04/test05_02/DA_TP.mat','dxb','dxf','dxc','dxbPSI','dxfPSI','dxcPSI')

            end; % kkk
    elseif DASALL==3
% % % elseif strcmp(DAS,'DA_CE');
    % the most important decision is which approach, 
    % in this particular application, we decide to use
    % solution ?. 
    % change continuity equation only
...
%        R0=0.0003;
    ...
    dxb=[];dxf=[];dxc=[];
    [dxc,kent04,rep,cent05,kent06]=DA_diff_incr_CE_more_output(dd,dt,half_dt_iter,sAenodr,A6,D12,norm01,lnodes,opnod,kki,HH,HT,A11,nA61,uncer,R);
%            [dxc]=DA_diff_incr_CE(dd,dt,half_dt_iter,sAenodr,A6,D12,norm01,lnodes,opnod,kki,HH,HT,A11,nA61,uncer,R);

    dxbPSI=[];dxfPSI=[];dxcPSI=[];
	[dxbPSI]=LTE_fwd_BC_TP_CE(dxb,[],[],opnod,kki,A11,nA12,[],[]);
	[dxfPSI]=LTE_fwd_BC_TP_CE([],dxf,[],opnod,kki,A11,[],Af1,[]);
	[dxcPSI]=LTE_fwd_BC_TP_CE([],[],dxc,opnod,kki,A11,[],[],nA61);
    save('/disks/NASUSER/lshi/file_for_Ed/LTEs/unified/test_bc_in/code_2020_04_25/wcadcirc/a_2021_03_04/test05_02/DA_CE.mat','dxb','dxf','dxc','dxbPSI','dxfPSI','dxcPSI','dd','HH','kent04','rep','cent05','kent06','-v7.3');

            end;
    elseif DASALL==4
% % % elseif strcmp(DAS,'DA_BC_TP');
    % the most important decision is which approach, 
    % in this particular application, we decide to use
    % solution 4. 
...
%             dd=inpsi(:,2)-mpsi;
            ...
            % this is to calculate the adjustment applyed to boundary condition;
    dxb=[];dxf=[];dxc=[];
            [dxb,dxf]=DA_diff_incr_BC_TP(dd,dt,half_dt_iter,sAenodr,A6,D12,norm01,lnodes,opnod,kki,HH,HT,A11,nA12,Af1,uncerB,uncer,R);

    dxbPSI=[];dxfPSI=[];dxcPSI=[];
	[dxbPSI]=LTE_fwd_BC_TP_CE(dxb,[],[],opnod,kki,A11,nA12,[],[]);
	[dxfPSI]=LTE_fwd_BC_TP_CE([],dxf,[],opnod,kki,A11,[],Af1,[]);
	[dxcPSI]=LTE_fwd_BC_TP_CE([],[],dxc,opnod,kki,A11,[],[],nA61);
    %save('/disks/NASUSER/lshi/file_for_Ed/LTEs/unified/test_bc_in/code_2020_04_25/wcadcirc/a_2021_03_04/test05_02/DA_BC_TP.mat','dxb','dxf','dxc','dxbPSI','dxfPSI','dxcPSI')
    %save('out/DA_BC_TP.mat','dxb','dxf','dxc','dxbPSI','dxfPSI','dxcPSI')
    eval(['save ' pathout '/DA_BC_TP.mat dxb dxf dxc dxbPSI dxfPSI dxcPSI '])

            end; % kkk
    
    elseif DASALL==5
% % % elseif strcmp(DAS,'DA_BC_CE');
    % the most important decision is which approach, 
    % in this particular application, we decide to use
    % solution ?. 
    % change both the continuity equation and boundary forcing
...
%             dd=inpsi(:,2)-mpsi;
    ...
    dxb=[];dxf=[];dxc=[];
    [dxb,dxc]=DA_diff_incr_BC_CE(dd,dt,half_dt_iter,sAenodr,A6,D12,norm01,lnodes,opnod,kki,HH,HT,A11,nA12,nA61,uncerB,uncer,R);

    dxbPSI=[];dxfPSI=[];dxcPSI=[];
	[dxbPSI]=LTE_fwd_BC_TP_CE(dxb,[],[],opnod,kki,A11,nA12,[],[]);
	[dxfPSI]=LTE_fwd_BC_TP_CE([],dxf,[],opnod,kki,A11,[],Af1,[]);
	[dxcPSI]=LTE_fwd_BC_TP_CE([],[],dxc,opnod,kki,A11,[],[],nA61);
    save('/disks/NASUSER/lshi/file_for_Ed/LTEs/unified/test_bc_in/code_2020_04_25/wcadcirc/a_2021_03_04/test05_02/DA_BC_CE.mat','dxb','dxf','dxc','dxbPSI','dxfPSI','dxcPSI')

            end;
end; % if strcmp(DAS,'DA_NO'); 
    end;% for DASALL=1:5;
%save /disks/NASUSER/lshi/file_for_Ed/LTEs/unified/test_bc_in/code_2020_04_25/wcadcirc/a_2021_03_04/test05_02/fwd_matrix.mat opnod kki A11 nA12 Af1 nA61;
%save out/fwd_matrix.mat opnod kki A11 nA12 Af1 nA61;
eval(['save ' pathout '/fwd_matrix.mat opnod kki A11 nA12 Af1 nA61']);
